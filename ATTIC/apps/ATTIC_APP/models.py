...
class Junk(models.Model):#ALSO REFFERED TO AS JUNK
    ...
    avgRating = models.FloatField(default=0)#AVERAGE RATING OF THE JUNK
...
class Booking(models.Model):#TRACK RESERVATION OF JUNK
    junk = models.ForeignKey(Junk, related_name='booked')#CONNECTION WITH JUNK
    user = models.ForeignKey(Users, related_name='booked')#CONNNECTION WITH USER
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
